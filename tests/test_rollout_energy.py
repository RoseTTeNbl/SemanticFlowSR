...
def test_rollout_beats_random_policy():
    ...
    # target task: a multiplicative composition (not a trivial single-column linear fit)
    ...
    for seed in range(5):
        ...
        e0.append(r1.energy_trace[0])
    # learned policy makes real progress (well below the initial residual energy) and is
    # not dramatically worse than a random policy on average
    assert sum(learned) / len(learned) < 0.5 * (sum(e0) / len(e0))
    assert sum(learned) / len(learned) <= 2.0 * (sum(rand) / len(rand)) + 1e-6
