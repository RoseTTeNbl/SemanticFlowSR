...
class SupportSampler:
    ...
    def sample(self, action_ids: torch.Tensor, rewards: torch.Tensor | None = None,
        ...
        if self.mode == "adaptive_full":
            threshold = self.full_threshold if self.full_threshold is not None else self.max_support
            if threshold is None or n <= int(threshold):
                return SupportSample(action_ids=action_ids,
                                     proposal_probs=torch.ones_like(action_ids, dtype=torch.float32),
                                     full_size=n, mode=self.mode)
            sample_mode = "mixed_topk_random"
        elif self.mode == "full" or self.max_support is None or n <= self.max_support:
        ...
        if sample_mode not in {"topk_reward", "mixed_topk_random", "proposal_importance"}:
        ...
        if sample_mode in {"topk_reward", "mixed_topk_random"}:
        ...
        if slots and remaining and sample_mode != "topk_reward":
            ...
            if sample_mode == "proposal_importance":
